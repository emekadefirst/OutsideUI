              ...
              {allImages.map((_, index) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    index === activeImageIndex ? 'bg-white w-8' : 'bg-white/50 w-2'
                  }`}
                />
              ))}
            </div>
              ...
              <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-2xl p-8 border border-white/10">
                ...
                <div className="mt-8 border-t border-white/10 pt-6">
                  ...
                  <div className="bg-white/5 rounded-xl overflow-hidden border border-white/10">
                    ...
                    >
                      <img
                        src={getGoogleMapsImageUrl()}
                        alt="Event Location Map"
                        className="w-full h-64 object-cover"
                        onError={(e) => {
                          // Fallback if map image fails to load
                          e.target.src = `https://maps.googleapis.com/maps/api/staticmap?center=${encodeURIComponent(event.address)}&zoom=15&size=600x300&maptype=roadmap&markers=color:red%7C${encodeURIComponent(event.address)}&key=YOUR_GOOGLE_MAPS_API_KEY`;
                        }}
                      />
                    </a>
                    <div className="p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-white font-medium">{event.address}</p>
                          <p className="text-white/70 text-sm">Click map to open in Google Maps</p>
                        </div>
